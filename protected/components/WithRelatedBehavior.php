...
class WithRelatedBehavior extends CActiveRecordBehavior
{
    ...
    public function validate($data=null, $clearErrors=true, $owner=null)
    {
        ...
        foreach ($newData as $name => $data)
        {
            ...
            if (!is_array($related)) //has multiple related records?
                $related = array($related);
                
            foreach ($related as $model)
            {
                if (is_array($data))
                    $valid = $this->validate($data, $clearErrors, $model) && $valid; //validate with children
                else
                    $valid = $model->validate(null, $clearErrors) && $valid; //validate without children
            }
     ...
     */
    public function save($data=null, $runValidation=true)
    {
    ...
    private function internalSave($data=null, $owner=null)
    {
        ...
        try
        {
            ...
            //Save the owner! (or delete it)
            if($owner->markedDeleted && !$owner->isNewRecord)
            {
                return $owner->delete();
            }
            else if(!$owner->markedDeleted)
            {
                ...
                switch ($relationClass)
                {
                    ...
                    case CActiveRecord::HAS_MANY:
                        ...
                        foreach ($related as $model)
                        {
                            ...
                            if ($data === null)
                            {
                                //Saving the hasMany (or delete it)
